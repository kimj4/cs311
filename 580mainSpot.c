 ...
 * Carleton College
 * CS 311
 * adapted from code by Josh Davis
 * demonstrates spotlight capabilities.
 * WASD keys move the light in the XY plane
 * 1 and 2 keys move the light in the Z axis.
 */
...
texTexture texture3;
GLdouble lightPhi = 0;
GLdouble lightTheta = 0;
GLdouble lightPos[3] = {0, 0, 5};
...
void handleKey(GLFWwindow *window, int key, int scancode, int action,
	...
	if (action == GLFW_PRESS && key == GLFW_KEY_L) {
		...
		if (key == GLFW_KEY_O)
			...
			lightShineFrom(&light, lightPos, lightPhi , lightTheta);
		} else if (key == GLFW_KEY_D) {
			lightPos[0] = lightPos[0] + .1;
			lightShineFrom(&light, lightPos, lightPhi , lightTheta);
		}	else if (key == GLFW_KEY_W) {
			lightPos[1] = lightPos[1] + .1;
			lightShineFrom(&light, lightPos, lightPhi , lightTheta);
...
int initializeScene(void) {
	...
	// set the first textures to be something else for each node
...
int initializeShaderProgram(void) {
    ...
    void main() {\
        ...
        vec3 specLight = pow(specInt / a, shininess) * lightCol * specular;\
				if (lightCos <= dot(litDir, lightDir)) {\
					gl_FragColor = vec4(diffLight + specLight, 1.0);\
				} else {\
					gl_FragColor = 0.0 * vec4(diffLight + specLight, 1.0);\
				}\
    }";
	...
	if (program != 0) {
		...
		unifLocs[6] = glGetUniformLocation(program, "lightDir");
		unifLocs[7] = glGetUniformLocation(program, "lightCos");
...
void render(void) {
	...
	lightRender(&light, unifLocs[3], unifLocs[4], unifLocs[5], unifLocs[6], unifLocs[7]);
...
int main(void) {
    ...
    // Initialize all the textures and put them into the textures list.
		...
		GLdouble lightAtt[3] = {1.0, 0.0, 0.0}; // turn off attenuation
		GLuint lightType = lightSPOT;
		GLdouble lightAngle = M_PI / 8;

		lightSetTranslation(&light, lightPos);
		lightSetColor(&light, lightRGB);
		lightSetAttenuation(&light, lightAtt);
		lightSetType(&light, lightType);
