...
int initializeMesh(void) {
	int error;
	meshMesh mesh;
	// jk: this is just some mesh that isn't in GL standard
	error = meshInitializeCapsule(&mesh, 0.5, 2.0, 16, 32);
	if (error != 0)
		return 1;
	// jk: here we pass in the created mesh to make it GL standard
	meshGLInitialize(&meshGL, &mesh);
	// jk: the non-gl mesh is useless now
	meshDestroy(&mesh);
	return 0;
}
...
int main(void) {
    ...
    if (window == NULL) {
		...
		// jk: end error checking
    glfwSetWindowSizeCallback(window, handleResize);
    glfwMakeContextCurrent(window);
		// jk: prints version number stuff
    fprintf(stderr, "main: OpenGL %s, GLSL %s.\n",
						glGetString(GL_VERSION), glGetString(GL_SHADING_LANGUAGE_VERSION));
		// jk: enable some GL features
